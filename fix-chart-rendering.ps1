# PowerShell script to fix chart rendering issues
# This script will:
# 1. Create a new, simplified chart component
# 2. Update the TableSection component to use the new chart
# 3. Restart the development server

function Create-Basic-Chart-Component {
  $chartComponentPath = "src/components/basic-chart.tsx"

  Write-Host "Creating simplified chart component at $chartComponentPath..."
...
"@
    ...
    # Create the file
    $chartComponentContent | Out-File -FilePath $chartComponentPath -Encoding utf8

    Write-Host "Chart component created successfully!"
}

function Update-Table-Section {
    $tableSectionPath = "src/components/table-section.tsx"

    Write-Host "Updating TableSection component to use the new chart..."

    # Read the current content
    $content = Get-Content -Path $tableSectionPath -Raw

    # Update the import statement
    $content = $content -replace "import \ { SimpleBarChart \ } from ""./simple-bar-chart""; ", "import { BasicChart } from ""./basic-chart""; "

    # Update the chart rendering
    $content = $content -replace "<SimpleBarChart\s+taxEntries=\ { taxEntries\ }\s+calculateTaxRevenue=\ { calculateTaxRevenue\ }\s+calculateTaxRevenueWithUBI=\ { calculateTaxRevenueWithUBI\ }\s+calculateMedianTaxWithUBI=\ { calculateMedianTaxWithUBI\ }\s+calculateUBICost=\ { calculateUBICost\ }\s+calculateIncomeWithUBI=\ { calculateIncomeWithUBI\ }\s+/>", "<BasicChart taxEntries= { taxEntries } calculateIncomeWithUBI= { calculateIncomeWithUBI } />"

    # Write the updated content back to the file
    $content | Out-File -FilePath $tableSectionPath -Encoding utf8

    Write-Host "TableSection component updated successfully!"
}

function Restart-Dev-Server {
    Write-Host "Restarting development server..."

    # Find the npm process running the dev server
    $npmProcess = Get-Process -Name "npm" -ErrorAction SilentlyContinue

    if ($npmProcess) {
        Write-Host "Stopping existing npm process..."
        $npmProcess | Stop-Process -Force
        Start-Sleep -Seconds 2
    }

    # Start the dev server in a new window
    Write-Host "Starting new dev server..."
    Start-Process "npm" -ArgumentList "run dev" -WorkingDirectory "."

    Write-Host "Development server restarted!"
}

# Execute the functions
Create-Basic-Chart-Component
Update-Table-Section
# Restart-Dev-Server  # Uncomment this line if you want to restart the server automatically

Write-Host "Chart rendering fix applied! Please refresh your browser to see the changes."
