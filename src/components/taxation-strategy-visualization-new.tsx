...
import type { TaxationModel } from "../models/taxation-models";
import { calculateTax } from "../models/taxation-models";
...
export const TaxationStrategyVisualization =
  component$<TaxationStrategyVisualizationProps>(({ model }) => {
    // Pre-calculate tax rates
    ...
    const initialTaxRates = sampleIncomes.map((income) => {
      ...
      const fallbackTaxRate = 30; // 30% flat tax rate
      const tax = calculateTax(income, model.id, exemptionAmount, fallbackTaxRate);
    ...
    return (
      <div class="taxation-strategy-visualization">
        <h3 class="strategy-name">{model.name} Model</h3>
        <p class="strategy-description">{model.description}</p>
