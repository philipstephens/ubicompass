...
export const UbiCalculationRefactored = component$(() => {
  ...
  const forceUpdate = useSignal(0); // Signal to force re-renders
  ...
  // Track changes to the exemption amount
  useTask$(({ track }) => {
    // Track the exemption amount
    track(() => ubiStore.selectedExemptionAmount);
    // Also track the force update signal
    track(() => forceUpdate.value);
    ...
    // Force a re-render of the income range component
    // This ensures the tax brackets display is updated
    if (ubiStore.selectedExemptionAmount !== undefined) {
      // The UI will update automatically because we're using
      // the selectedExemptionAmount directly in the JSX
      console.log(
        "Income range component should update with new exemption amount:",
        ubiStore.selectedExemptionAmount
      );
    }
  });

  // Track changes to the flat tax percentage
  useTask$(({ track }) => {
    ...
    console.log(
      "Flat tax percentage changed to:",
      ubiStore.selectedYear?.flattaxpercentage
    );

    // Force a re-render of the income range component
    // This ensures the tax brackets display is updated
    if (ubiStore.selectedYear?.flattaxpercentage !== undefined) {
      // The UI will update automatically because we're using
      // the flat tax percentage directly in the JSX
      console.log(
        "Income range component should update with new flat tax percentage:",
        ubiStore.selectedYear?.flattaxpercentage
      );
    }
  });
  ...
  useTask$(({ track }) => {
    ...
    try {
      ...
      const taxModel = TaxModelFactory.createModelById(
        modelId,
        flatTaxPercentage
      );
      currentTaxModel.value = noSerialize(taxModel);
      ...
      console.error("Error creating tax model:", error);
    }
  });
    ...
    try {
      ...
      if (ubiStore.selectedModelId === 1) {
        ...
        try {
          ...
          console.error("Error calculating quintile revenue:", err);
          // Fallback to flat tax calculation
          const taxRate = flatTaxPercentage / 100;
          const revenue = income * taxRate * taxpayers;
          return Math.round(revenue * 10) / 10;
        }
      ...
      console.error("Error in calculateTaxRevenue:", error);
      return 0;
    }
  });
  ...
  const calculateTaxRevenueWithUBI = $((entry: any) => {
    try {
      ...
      const income = entry.averagetaxableincome;
      const ubiAmount =
        ((ubiStore.selectedYear?.ubiamount || 2000) * 12) / 1000;
      ...
      if (ubiStore.selectedModelId === 1) {
        ...
        try {
          ...
          console.error("Error calculating quintile revenue with UBI:", err);
          // Fallback to flat tax calculation
          const taxRate = flatTaxPercentage / 100;
          const taxableIncomeWithUBI = Math.max(
            0,
      ...
      console.error("Error in calculateTaxRevenueWithUBI:", error);
      return 0;
    }
  });
    ...
    try {
      ...
      if (ubiStore.selectedModelId === 1) {
        ...
        try {
          ...
          console.error("Error calculating tax with UBI:", err);
          // Fallback to flat tax calculation
          const taxRate = flatTaxPercentage / 100;
          const taxableIncomeWithUBI = Math.max(
            0,
      ...
      console.error("Error in calculateMedianTaxWithUBI:", error);
      return 0;
    }
  });
    ...
    try {
      ...
      if (ubiStore.selectedModelId === 1) {
        ...
        try {
          ...
          console.error("Error calculating UBI cost:", err);
          // Fallback to flat tax calculation
          // Calculate UBI payments to this quintile
          const ubiPayments = taxpayers * ubiAmount;
      ...
      console.error("Error in calculateUBICost:", error);
      return 0;
    }
  });
  ...
  const getTaxBrackets = $(() => {
    try {
      ...
      // Return default flat tax brackets
      return [
        {
          rate: 0,
          minIncome: 0,
          maxIncome: ubiStore.selectedExemptionAmount || 24,
          description: "No tax on income below exemption",
        },
        {
          rate: ubiStore.selectedYear?.flattaxpercentage || 30,
          minIncome: ubiStore.selectedExemptionAmount || 24,
          description: "Flat tax rate on income above exemption",
        },
      ];
    }
  });
  ...
  console.log("UbiCalculationRefactored: About to render component");
  ...
  return (
    <div style={{ border: "2px solid green", padding: "10px" }}>
      ...
      >
        Debug: UbiCalculationRefactored Component
      </div>
      <CspCardComponent
        title={
                  ...
                  >
                    ...
                    <div style={{ padding: "20px" }}>
                      ...
                      >
                        ...
                        >
                          <span>
                            {/* Force this to update when exemption amount changes */}
                            {forceUpdate.value > 0
                              ? `$0k - $${ubiStore.selectedExemptionAmount || 24}k`
                              : `$0k - $${ubiStore.selectedExemptionAmount || 24}k`}
                          </span>
                      ...
                      >
                        ...
                        >
                          <span>
                            {/* Force this to update when exemption amount changes */}
                            {forceUpdate.value > 0
                              ? `$${ubiStore.selectedExemptionAmount || 24}k+`
                              : `$${ubiStore.selectedExemptionAmount || 24}k+`}
                          </span>
                          <span style={{ fontWeight: "bold" }}>
                            {/* Force this to update when flat tax percentage changes */}
                            {forceUpdate.value > 0
                              ? `${ubiStore.selectedYear?.flattaxpercentage || 30}%`
                              : `${ubiStore.selectedYear?.flattaxpercentage || 30}%`}
                          </span>
