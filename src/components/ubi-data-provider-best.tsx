import { component$, useContextProvider, useStore, useTask$, Slot } from "@builder.io/qwik";
...
import { getMockTaxationModels, getMockTaxationModelData } from "../services/taxation-service";
...
export const UbiDataProviderBest = component$(() => {
  ...
  useTask$(() => {
    ...
    if (ubiDataStore.yearData.length > 0) {
      ...
      ubiDataStore.taxationModelData = getMockTaxationModelData(1, parseInt(firstYearId));
    }
  ...
  useTask$(({ track }) => {
    ...
    // Update selectedYear
    ubiDataStore.selectedYear = ubiDataStore.yearData.find(
      (year) => year.ubiid.toString() === yearId
    ) || null;
    ...
    // Load tax entries
    ubiDataStore.taxEntries = mockTaxEntries.map(entry => ({
      ...entry,
  ...
  useTask$(({ track }) => {
    ...
    // Load taxation model data
    ubiDataStore.taxationModelData = getMockTaxationModelData(modelId, parseInt(yearId));
  });
